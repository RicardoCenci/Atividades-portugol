programa
{
	...
	funcao inicio()
	{
		...
		para(inteiro POSICAO=0 ; POSICAO < 11 ; POSICAO++){
			leia(CPF[POSICAO])	//LE O NUMERO DO CPF NA POSIÇÃO
		}
		
		se( verificaCPF(CPF) ){		//Passa o CPF para a função verificaCPF que retornara um booleano
			escreva("CPF Valido") 
		}senao{ 
	...
	funcao logico verificaCPF(inteiro CPF[]){
		...
		para( inteiro NUMteste=10 ; VEZES < 9 ; NUMteste-- ){	//MULTIPLICA E SOMA AS 9 PRIMEIRAS POSIÇÕES
			RESTO = RESTO + CPF[VEZES] * NUMteste
			VEZES++ 		//VEZES++ é a mesma coisa que VEZES = VEZES + VEZES
		}

		RESTO = RESTO * 10 //Multiplica td isso por 10
		
		//Caso o calculo der 10, ele precisa transformar o 10 em 0 para testar 
		se(RESTO % 11 == 10){ 
			TEMPORARIO=CPF[9] 	//guarda o numero da posição 9
			CPF[9]=10		//Define a posição 9 como 10
		}

		se(RESTO % 11 != CPF[9]){	//Testa se o resto de divisão por 11 é igual a posição 9, se for igual, prossegue com o algoritimo, se não retorna falso
			retorne falso
		}
	
		VEZES=0				//reseta o VEZES pro proximo teste
		
		se(RESTO % 11 == 10){		//Retorna a posição 9 ao valor original
			CPF[9] = TEMPORARIO
		}
	
		RESTO = 0			//Reseta o valor do RESTO para o proximo teste

		//Testa, agora com 11 numeros incluindo o 10
		para(inteiro NUMteste=11 ; VEZES < 10 ; NUMteste--){
			RESTO=RESTO+CPF[VEZES]*NUMteste
			VEZES++
		}
		...
		se(CPF[1]==CPF[2] e CPF[2]==CPF[3] e CPF[3]==CPF[4]){	//Retorna FALSO se os primeiros digitos forem iguais
			retorne falso
		}

		
		se(RESTO % 11 != CPF[10]){	//Testa se o valor do resto de divisão por 11 é o valor do vetor na posição 10
			retorne falso
		}senao{
			retorne verdadeiro
		}
